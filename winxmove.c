...
void (*hook_set_resize_modifier)(int);
...
int WINAPI WinMain(HINSTANCE pid, HINSTANCE prev, char *args, int show)
{
	...
	hook_set_resize_modifier = GetProcAddress(dll, "hook_set_resize_modifier");

	if(strstr(args, "-m")) {
		res_middle = 1;
		hook_set_buttons(VK_LMENU, 1, 2);
	} else {
		hook_set_buttons(VK_LMENU, 1, 1);
	}
	
	if(hook_set_resize_modifier) {
		hook_set_resize_modifier(VK_LCONTROL);
	}
