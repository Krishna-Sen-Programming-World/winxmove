...
int res_dir SHARED;
int mod_key SHARED = VK_LMENU, move_bn SHARED = 1, res_bn SHARED = 1;
int res_mod_key SHARED = VK_LCONTROL;
void *win SHARED;
POINT prev_pt SHARED;
#elif defined(_MSC_VER)
#pragma data_seg(".shared")
int moving, resizing, res_dir;
int mod_key = VK_LMENU, move_bn = 1, res_bn = 1;
int res_mod_key = VK_LCONTROL;
POINT prev_pt;
void *win;
#pragma data_seg()
#else
...
__declspec(dllexport) void hook_set_resize_modifier(int res_mod)
{
	res_mod_key = res_mod;
}
...
{
	POINT pt;
	int modkey_state, res_modkey_state, msg = wpar;
	int handled = 0;
	...
	res_modkey_state = GetAsyncKeyState(res_mod_key);
	
	switch(msg) {
	case WM_MOUSEMOVE:
		if(moving) {
			/* Check if Alt key is still pressed for moving */
			if(!GetAsyncKeyState(mod_key)) {
				moving = 0;
			} else {
			...
			/* Check if Alt key and appropriate mouse button are still pressed for resizing */
			if(!GetAsyncKeyState(mod_key) || (!GetAsyncKeyState(VK_LBUTTON) && !GetAsyncKeyState(VK_RBUTTON))) {
				resizing = 0;
			} else {
	...
	case WM_RBUTTONDOWN:
	case WM_MBUTTONDOWN:
		if(res_modkey_state && modkey_state && msg == get_down_event(res_bn)) {
			/* Ctrl + Alt + Left Mouse for resizing */
			RECT rect;
			resizing = 1;
			prev_pt = pt;
			handled = 1;
		...
		} else if(modkey_state && msg == WM_RBUTTONDOWN) {
			/* Alt + Right Mouse for resizing */
			RECT rect;
			resizing = 1;
			prev_pt = pt;
			handled = 1;
			...
			GetWindowRect(win, &rect);
			res_dir = get_res_dir(pt.x, pt.y, rect.left, rect.top, rect.right, rect.bottom);
		} else if(modkey_state && msg == get_down_event(move_bn)) {
			/* Alt + Left Mouse for moving */
			moving = 1;
			prev_pt = pt;
			handled = 1;
			win = get_toplevel(WindowFromPoint(pt));
	...
	case WM_MBUTTONUP:
		if(msg == get_up_event(move_bn)) {
			...
			moving = 0;
		} else if(msg == get_up_event(res_bn) || msg == WM_RBUTTONUP) {
			if(resizing) {
				handled = 1;
			}
			resizing = 0;
